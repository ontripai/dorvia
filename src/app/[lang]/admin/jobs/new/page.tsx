...
export default function NewJobListingPage({ params }: { params: { lang: Language } }) {
  ...
  const [city, setCity] = useState('بخارست');
  const [salaryMin, setSalaryMin] = useState<string>('3000');
  const [salaryMax, setSalaryMax] = useState<string>('4000');
  ...
  // Auto-generate Persian slug when title changes
  const handleTitleFaChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const val = e.target.value;
    setTitleFa(val);
    if (!slugFa || slugFa === slugifyJob(titleFa)) {
      setSlugFa(slugifyJob(val));
    }
  };
  ...
  const handleTitleEnChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const val = e.target.value;
    setTitleEn(val);
    if (!slugEn || slugEn === slugifyJob(titleEn)) {
      setSlugEn(slugifyJob(val));
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/${currentLang}/admin/jobs`);
    } catch (err: any) {
      setError(err.message || 'An error occurred while creating the job listing');
    } finally {
      setSubmitting(false);
    }
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl sm:text-2xl font-black text-white">
                {isFa ? 'ثبت موقعیت شغلی جدید' : 'New Job Opportunity'}
              </h1>
              <p className="text-xs text-slate-400">
                {isFa
                  ? 'ثبت اولیه آگهی به صورت پیش‌نویس در سامانه'
                  : 'New listings are automatically saved as draft'}
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input
                  ...
                  required
                  value={titleFa}
                  onChange={handleTitleFaChange}
                  placeholder={isFa ? 'مثال: استادکار آرماتوربندی و قالب‌بندی' : 'e.g. Concrete Formwork Carpenter'}
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="concrete-carpenter-bucharest"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder={isFa ? 'بخارست، کلوژ، تیمیشوارا...' : 'Bucharest, Cluj-Napoca...'}
                  className="w-full px-4 py-2.5 rounded-xl bg-slate-950 border border-slate-800 text-slate-100 text-sm focus:outline-none focus:border-blue-500 transition-all"
                />
              </div>
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <textarea
                    ...
                    placeholder={isFa ? 'شرح کامل کار، وظایف روزانه، ساعات کاری و مزایا...' : 'Describe job responsibilities and daily routine...'}
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder={isFa ? '- حداقل ۲ سال سابقه کار مرتبط\n- توانایی کار تیمی\n- عدم سوءپیشینه' : '- Minimum 2 years experience\n- Teamwork capability'}
                    className="w-full px-4 py-3 rounded-xl bg-slate-950 border border-slate-800 text-slate-100 text-sm focus:outline-none focus:border-blue-500 font-sans leading-relaxed"
                  />
                </div>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">
                  {isFa
                    ? 'برای انتشار نسخه انگلیسی در سایت، هر سه فیلد Title، Slug و Description باید با هم تکمیل باشند.'
                    : 'All three fields (Title, Slug, Description) are strictly required for the English page.'}
                </p>
              </div>
              <button
                type="button"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g. Master Construction Formwork Carpenter"
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="master-formwork-carpenter-bucharest"
          ...
          <div className="flex items-center justify-end gap-4 pt-4 border-t border-slate-800">
            ...
            >
              {submitting
                ...
                : (isFa ? 'ذخیره به صورت پیش‌نویس' : 'Save as Draft')}
            </button>
          </div>
        </form>
      </div>
