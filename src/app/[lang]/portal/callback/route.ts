...
export async function GET(request: Request, { params }: { params: { lang: string } }) {
  ...
  // When Supabase generates implicit magic links or invite links (#access_token=...&refresh_token=...),
...
<head>
  <meta charset="utf-8" />
  <title>${lang === 'fa' ? 'در حال ورود به پورتال...' : 'Authenticating Case Portal Session...'}</title>
...
<body>
  <div style="text-align: center; padding: 24px;">
    ...
    <div style="font-size: 14px; font-weight: bold; color: #526174;">
      ${lang === 'fa' ? 'در حال اعتبارسنجی ورود به پرونده...' : 'Validating case portal session...'}
    </div>
  </div>
  <script>
    (async function() {
