...
export const TelecomGuideContent: React.FC<TelecomGuideContentProps> = ({ currentLang, onNavigate }) => {
  ...
  const telecomFaqs = [
    ...
    {
      q: currentLang === 'fa' ? 'آیا در رومانی eSIM موجود است و بدون قرارداد هم می‌شود گرفت؟' : 'Is eSIM available in Romania, and can I get one without a contract?',
      a: currentLang === 'fa'
        ? 'بله. Orange روی سایت خودش اعلام کرده که eSIM را می‌توان روی سیم‌کارت اعتباری (PrePay) هم فعال کرد و صفحه‌ی راهنمای جداگانه‌ای برای «eSIM PrePay» دارد. Digi هم صفحه‌ی رسمی eSIM پیش‌پرداخت دارد. Vodafone علاوه بر eSIM معمولی، سرویسی به نام Vodafone EASY دارد که خودش آن را «سرویس موبایل مبتنی بر eSIM، کاملاً دیجیتال، بدون دوره‌ی قراردادی» توصیف می‌کند. چرا این تمایز مهم است: تازه‌واردی که هنوز کارت اقامت یا حساب بانکی رومانیایی ندارد معمولاً نمی‌تواند قرارداد ماهانه (abonament) بگیرد. پس صرفِ اینکه «رومانی eSIM دارد» به کارتان نمی‌آید — چیزی که به کارتان می‌آید این است که کدام گزینه بدون قرارداد فعال می‌شود. توجه: ما نتوانستیم تایید کنیم که سیم‌کارت اعتباری کلاسیک Vodafone (cartelă) هم به‌صورت eSIM صادر می‌شود؛ آنچه تایید شد سرویس جداگانه‌ی EASY است. با توجه به تغییرات مداوم طرح‌ها و پیشنهادهای اپراتورها، حتماً شرایط روز را پیش از اقدام مستقیماً در وب‌سایت رسمی همان اپراتور بررسی فرمایید.'
        : 'Yes. Orange states on its own site that eSIM can be activated on a prepaid (PrePay) card, and maintains a dedicated "eSIM PrePay" help page. Digi has an official prepaid eSIM page. Vodafone, besides standard eSIM, runs a service called Vodafone EASY which it describes as "a mobile service with eSIM, 100% digital, with no contract period". Why this distinction matters: a newcomer who doesn\'t yet have a residence card or a Romanian bank account usually cannot take out a monthly contract (abonament). So "Romania has eSIM" isn\'t the useful fact — what\'s useful is which option works without a contract. Note: We could not confirm that a classic Vodafone prepaid card (cartelă) can be issued as an eSIM; what we confirmed is the separate EASY service. Because operator offers and service terms change frequently, always verify current details directly on the carrier\'s official website before proceeding.'
    },
    {
      q: currentLang === 'fa' ? 'در فرودگاه بخارست می‌شود سیم‌کارت خرید؟' : 'Can I buy a SIM card at Bucharest airport?',
      a: currentLang === 'fa'
        ? 'بله. طبق سایت رسمی فرودگاه‌های بخارست، سیم‌کارت از فروشگاه Relay قابل خرید است که در راهروی رابط بین ترمینال ورودی و ترمینال خروجی قرار دارد. برای شارژ هم سه گزینه اعلام شده: همان فروشگاه Relay، باجه‌ی پستی، و دستگاه پرداخت SELFPAY در سوپرمارکت Carrefour. توجه: سایت فرودگاه نام اپراتورهای موجود در آن فروشگاه و قیمت‌ها را ذکر نکرده است. ما نتوانستیم از هیچ منبع معتبری تایید کنیم که قیمت فرودگاه با قیمت داخل شهر تفاوت دارد یا نه — پس درباره‌ی گران‌تر یا ارزان‌تر بودنش ادعایی نمی‌کنیم. اگر جایی خواندید که خرید در فرودگاه «حتماً گران‌تر» است، آن ادعا را ما نتوانستیم تایید کنیم.'
        : 'Yes. According to Bucharest Airports\' official site, SIM cards can be purchased at the Relay shop, located in the connecting corridor between the Arrivals and Departures terminals. For top-ups, three options are listed: the same Relay shop, the postal counter, and the SELFPAY payment machine in the Carrefour supermarket. Note: The airport\'s site does not name which operators that shop carries, nor any prices. We could not confirm from any credible source whether airport prices differ from city prices — so we make no claim either way. If you read elsewhere that buying at the airport is "definitely more expensive", that is a claim we could not verify.'
    },
    {
      q: currentLang === 'fa' ? 'می‌توانم خط ایرانی‌ام را فعال نگه دارم تا پیامک رمز یکبارمصرف بانک‌های ایران را در رومانی دریافت کنم؟' : 'Can I keep my Iranian number active to receive one-time-passcode SMS from Iranian banks while in Romania?',
      a: currentLang === 'fa'
        ? 'ما نتوانستیم این را تایید کنیم و توصیه می‌کنیم روی آن حساب باز نکنید. صفحه‌ی رومینگ ایرانسل از پوشش «بیش از ۲۲۳ اپراتور در ۱۳۳ کشور» می‌گوید، اما هیچ اپراتور شریکی در رومانی را نام نمی‌برد. برای همراه اول هم فهرست کشورهای تحت پوشش را پیدا نکردیم. هیچ‌کدام از این دو اپراتور، تحویل پیامک ورودی در حالت رومینگ را تضمین نمی‌کنند — و پیامک ورودی دقیقاً همان چیزی است که رمز بانکی شماست. سه نکته‌ی فنی که معمولاً نادیده گرفته می‌شوند: ۱) پیامک ورودی می‌تواند شکست بخورد حتی در جایی که تماس و اینترنت رومینگ درست کار می‌کند؛ این‌ها مسیرهای فنی متفاوتی دارند؛ ۲) رومینگ معمولاً باید پیش از خروج از ایران فعال شود، نه بعد از رسیدن؛ ۳) تحریم‌ها دامنه‌ی قراردادهای رومینگ اپراتورهای ایرانی را محدود می‌کنند و این فهرست‌ها بدون اطلاع قبلی تغییر می‌کنند — پس حتی تجربه‌ی موفق چند ماه پیش دیگری، هیچ تضمینی برای امروز شما نیست. چه کاری واقعاً مفید است: پیش از مهاجرت رومینگ را فعال کنید و در عمل تست کنید که پیامک بانکی می‌رسد یا نه. و مهم‌تر: اگر بانک شما روش ورود جایگزین دارد (رمزساز داخل اپلیکیشن، توکن سخت‌افزاری)، پیش از رفتن به آن منتقل شوید. روی پیامک بین‌المللی به‌عنوان تنها راه دسترسی به حساب بانکی‌تان تکیه نکنید.'
        : 'We could not confirm this, and we recommend not relying on it. Irancell\'s roaming page claims coverage of "over 223 operators across 133 countries", but names no partner operator in Romania. For MCI (Hamrah-e Aval) we could not locate a country coverage list at all. Neither operator guarantees delivery of inbound SMS while roaming — and inbound SMS is exactly what your banking passcode is. Three technical points that are usually overlooked: 1) Inbound SMS can fail even where roaming calls and data work fine; they travel different technical paths; 2) Roaming usually has to be activated before you leave Iran, not after you arrive; 3) Sanctions constrain which carriers will contract with Iranian operators, and those partner lists change without notice — so someone else\'s successful experience a few months ago is not a guarantee for you today. What actually helps: activate roaming before you travel and test in practice whether bank SMS arrives. More importantly, if your bank offers an alternative login method (an in-app passcode generator, a hardware token), switch to it before you leave. Do not rely on international SMS as your only route into your bank account.'
    }
  ];
