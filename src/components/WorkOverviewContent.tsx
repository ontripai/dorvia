        ...
        <div className="space-y-10 animate-fadeIn max-w-[1280px] mx-auto px-4 py-8">
          ...
          <div className="bg-[#071B3D] text-white rounded-2xl p-6 sm:p-8 space-y-4">
            ...
            <p className="text-sm text-slate-300 leading-relaxed">
              {currentLang === 'fa'
                ? 'اگر مدرک تحصیلات تکمیلی یا سابقه کاری تخصصی دارید، مسیر کارت آبی معمولاً سریع‌تر و با مزایای بیشتری همراه است. طبق قانون ۲۸/۲۰۲۴ (اجرایی از مارس ۲۰۲۴)، حداقل حقوق ناخالص لازم برای واجد شرایط بودن از «۲ برابر میانگین حقوق ناخالص کشوری» به «۱ برابر میانگین حقوق ناخالص کشوری» کاهش یافت. نکته مهم: دو رقم متفاوت برای «میانگین حقوق ناخالص کشوری» در منابع رسمی رومانی منتشر می‌شود و متن قانون به‌روشنی مشخص نکرده کدام‌یک مبنای محاسبه این آستانه است — (۱) رقم واقعی و ماهانه مرکز آمار ملی (INS) که برای ژوئن ۲۰۲۶ حدود ۹,۵۶۴ لئو بود؛ (۲) رقم ثابت سالانه‌ای که در قانون بودجه تامین اجتماعی ۲۰۲۶ برای محاسبه حق بیمه/مستمری تعیین شده و ۹,۱۹۲ لئو است. پیش از اقدام، حتماً از IGI یا کارفرمای خود بپرسید کدام رقم را برای این آستانه خاص در نظر می‌گیرند.'
                : 'If you hold an advanced degree or specialized professional experience, the EU Blue Card route is typically faster and comes with more benefits. Under Law 28/2024 (effective March 2024), the minimum gross salary threshold to qualify was reduced from "2× the average national gross salary" to "1× the average national gross salary." Important caveat: two different "average national gross salary" figures circulate in official Romanian sources, and it is not clearly specified which one the law\'s threshold is based on — (1) the real, monthly-measured figure from the National Statistics Institute (INS), which was around 9,564 RON for June 2026; (2) a fixed annual figure set by the 2026 state social-insurance budget law for calculating contributions/pensions, which is 9,192 RON. Before applying, confirm directly with IGI or your employer which figure they use for this specific threshold.'}
            </p>
          ...
          <div className="bg-white p-6 sm:p-8 rounded-2xl border border-[#dfe6ef] shadow-sm space-y-5">
            ...
            <div className="p-4 bg-red-50 border border-red-200 rounded-xl text-xs sm:text-sm text-red-950 leading-relaxed">
              <strong className="block font-bold mb-1">{currentLang === 'fa' ? 'به‌روزرسانی (اواخر اوت ۲۰۲۶): وضعیت هنوز حل‌نشده و در حال مناقشه است' : 'Update (late August 2026): the situation remains unresolved and disputed'}</strong>
              {currentLang === 'fa'
                ? 'طبق گزارش‌های اقتصادی رومانی (اواخر اوت ۲۰۲۶)، مشکلات راه‌اندازی فقط تاخیر ساده نبوده‌اند: کارفرمایان و آژانس‌هایی که پیش از راه‌اندازی پلتفرم، Aviz de Muncă قدیمی را دریافت کرده و هزینه آن (~۱۰۰ یورو در هر پرونده) را پرداخته بودند، گزارش می‌دهند سفارت‌ها/MAE اکنون همان مدارک را پس از راه‌اندازی رد می‌کنند — یعنی خطر ازدست‌رفتن کامل هزینه پرداخت‌شده. همچنین اینکه آیا پنجره ۱۸۰ روزه درخواست ویزا (که برای مجوزهای صادرشده تحت قوانین قدیمی تا حدود آوریل ۲۰۲۷ اعتبار داشت) پس از تغییر سامانه در ۸ آگوست ۲۰۲۶ همچنان معتبر است یا نه، هنوز از نظر حقوقی روشن نشده. فدراسیون کارفرمایان استخدام‌کننده نیروی خارجی (PIFM) رسماً از وزارت امور خارجه/مرکز ملی ویزا خواستار شفاف‌سازی شده و تا تاریخ این گزارش پاسخی اعلام نشده بود. نتیجه عملی: اگر پرونده شما یا کارفرمایتان مربوط به دوره گذار است، وضعیت را مستقیماً و مکرر از کارفرما/وکیل مهاجرت پیگیری کنید — این یک مسئله حل‌شده نیست.'
                : "Per Romanian business-press reporting (late August 2026), the rollout problems go beyond simple delays: employers and agencies who obtained the old Aviz de Muncă before the platform launched, and paid its fee (~€100 per file), report that embassies/MAE are now rejecting those same documents post-launch — meaning a real risk of losing that fee entirely. It also remains legally unclear whether the 180-day visa-application window attached to permits issued under the old rules (valid into roughly April 2027) survived the August 8, 2026 platform cutover. PIFM (the federation representing employers of foreign labor) has formally petitioned the Foreign Ministry/National Visa Center for clarification, with no resolution reported as of this writing. Practical takeaway: if your case or your employer's falls in this transition window, follow up directly and repeatedly with your employer or immigration lawyer — this is not yet a settled matter."}
            </div>
            <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl text-xs sm:text-sm text-slate-700 leading-relaxed">
              <strong className="block font-bold mb-1 text-[#142033]">{currentLang === 'fa' ? 'نیاز به کمک در این فرآیند دارید؟' : 'Need help navigating this process?'}</strong>
              {currentLang === 'fa'
                ? 'با توجه به مشکلات راه‌اندازی بالا، برخی کارفرمایان و متقاضیان از شرکت‌های خصوصی کارگزاری نیروی کار و مشاوره مهاجرت کمک می‌گیرند — برای مثال '
                : 'Given the rollout problems described above, some employers and applicants turn to private recruitment and immigration-consulting firms for help — for example '}
              <a href="https://www.workingromania.com/" target="_blank" rel="noopener noreferrer" className="font-bold text-[#2F6FED] hover:underline">WorkingRomania.com</a>
              {currentLang === 'fa'
        ...
        <div className="space-y-10 animate-fadeIn max-w-[1280px] mx-auto px-4 py-8">
          ...
          <div className="mt-12 bg-[#F8FAFC] rounded-2xl p-6 sm:p-8 border border-[#e2e8f0]">
            ...
            <div className="space-y-6">
              ...
              <div>
                <h3 className="font-bold text-[#334155] mb-2">{currentLang === 'fa' ? 'سود معاملات ارز دیجیتال چطور مالیات می‌گیرد؟' : 'How is crypto trading profit taxed?'}</h3>
                <p className="text-sm text-[#475569]">{currentLang === 'fa' ? 'به‌عنوان «درآمد از سایر منابع» طبق ماده ۱۱۶ قانون مالیات (Legea 227/2015) محسوب می‌شود. نرخ تا پایان ۲۰۲۵، ۱۰٪ بود؛ طبق قانون جدید (Legea 239/2025)، از سال ۲۰۲۶ به ۱۶٪ افزایش یافته است. اگر مجموع سود سالانه زیر ۶۰۰ لی بماند، معاف است؛ در غیر این صورت کل مبلغ مشمول می‌شود (نرخ فعلی از منابع مشاوره مالیاتی رومانیایی؛ پیشنهاد می‌شود قبل از اظهارنامه با یک حسابدار تایید کنید).' : 'Treated as "income from other sources" under Art. 116 of the Fiscal Code (Legea 227/2015). The rate was 10% through 2025; under new Law 239/2025, it rose to 16% starting 2026. If cumulative annual gains stay under 600 RON, it\'s exempt; otherwise the full amount is taxable (current rate from Romanian tax advisory sources; we recommend confirming with an accountant before filing).'}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-sm text-[#475569]">{currentLang === 'fa' ? 'ثبت قرارداد اجاره نزد ANAF ظرف ۳۰ روز، تکلیف قانونی صاحبخانه است، نه مستاجر؛ عدم انجام آن ریسک مالیاتی و جریمه برای خودِ صاحبخانه دارد. درباره‌ی اینکه آیا قرارداد ثبت‌نشده هنوز برای ثبت آدرس نزد اداره مهاجرت قابل استفاده است یا خیر، منبع رسمی روشنی پیدا نکردیم — برای اطمینان، بهتر است در قرارداد اجاره‌تان اصرار کنید که صاحبخانه آن را نزد ANAF ثبت کند.' : 'Registering the rental contract with ANAF within 30 days is the landlord\'s legal obligation, not the tenant\'s; failing to do so creates tax risk for the landlord themselves. We didn\'t find a clear official source on whether an unregistered contract can still be used for your address registration with the immigration authority — to be safe, it\'s best to insist your landlord registers the contract with ANAF.'}</p>
              </div>
              <div>
                <h3 className="font-bold text-[#334155] mb-2">{currentLang === 'fa' ? 'می‌توانم مالیات بر ارزش افزوده (VAT/TVA) خریدهایم را هنگام خروج از رومانی پس بگیرم؟' : 'Can I get a VAT refund on purchases when leaving Romania?'}</h3>
                <p className="text-sm text-[#475569]">{currentLang === 'fa' ? 'بله، از طریق اپراتورهای مجاز «Tax Free» (مانند Ask for Tax Free) برای خریدهای بالای ۱۷۵ یورو (شامل مالیات) در هر فاکتور، به شرطی که ظرف ۹۰ روز کالا را از کشور خارج کنید و فاکتور را در گمرک مهر بزنید. کارمزد این اپراتورها حدود ۶٪ به‌علاوه‌ی هزینه‌ی بانکی از مبلغ بازپرداختی کسر می‌شود.' : 'Yes, through authorized "Tax Free" operators (e.g., Ask for Tax Free) for purchases over €175 (VAT included) per invoice, provided you export the goods within 90 days and get the invoice stamped at customs. These operators deduct roughly a 6% commission plus banking fees from the refund.'}</p>
              </div>
              <div>
                <h3 className="font-bold text-[#334155] mb-2">{currentLang === 'fa' ? 'کد CNP چیست و چطور آن را دریافت کنم؟' : 'What is a CNP and how do I get one?'}</h3>
                <p className="text-sm text-[#475569]">{currentLang === 'fa' ? 'کد CNP (کد عددی شخصی) شناسه یکتای هر فرد در رومانی است که به‌محض دریافت کارت اقامت، به‌طور خودکار به شما اختصاص داده می‌شود و روی خودِ کارت چاپ می‌شود — نیازی به درخواست جداگانه نیست. این کد برای امور مالیاتی، بانکی، درمانی و بسیاری از خدمات رسمی دیگر الزامی است.' : 'The CNP is your unique personal identification number in Romania, automatically assigned when you receive your residence card and printed on the card itself — no separate application is needed. It\'s required for tax, banking, healthcare, and many other official services.'}</p>
              </div>
            </div>
          </div>
          ...
          <FaqSchema items={[
            ...
            {
              q: currentLang === 'fa' ? 'سود معاملات ارز دیجیتال چطور مالیات می‌گیرد؟' : 'How is crypto trading profit taxed?',
              a: currentLang === 'fa' ? 'به‌عنوان «درآمد از سایر منابع» طبق ماده ۱۱۶ قانون مالیات (Legea 227/2015) محسوب می‌شود. نرخ تا پایان ۲۰۲۵، ۱۰٪ بود؛ طبق قانون جدید (Legea 239/2025)، از سال ۲۰۲۶ به ۱۶٪ افزایش یافته است. اگر مجموع سود سالانه زیر ۶۰۰ لی بماند، معاف است؛ در غیر این صورت کل مبلغ مشمول می‌شود (نرخ فعلی از منابع مشاوره مالیاتی رومانیایی؛ پیشنهاد می‌شود قبل از اظهارنامه با یک حسابدار تایید کنید).' : 'Treated as "income from other sources" under Art. 116 of the Fiscal Code (Legea 227/2015). The rate was 10% through 2025; under new Law 239/2025, it rose to 16% starting 2026. If cumulative annual gains stay under 600 RON, it\'s exempt; otherwise the full amount is taxable (current rate from Romanian tax advisory sources; we recommend confirming with an accountant before filing).'
            },
            {
            ...
            },
            {
              ...
              a: currentLang === 'fa' ? 'ثبت قرارداد اجاره نزد ANAF ظرف ۳۰ روز، تکلیف قانونی صاحبخانه است، نه مستاجر؛ عدم انجام آن ریسک مالیاتی و جریمه برای خودِ صاحبخانه دارد. درباره‌ی اینکه آیا قرارداد ثبت‌نشده هنوز برای ثبت آدرس نزد اداره مهاجرت قابل استفاده است یا خیر، منبع رسمی روشنی پیدا نکردیم — برای اطمینان، بهتر است در قرارداد اجاره‌تان اصرار کنید که صاحبخانه آن را نزد ANAF ثبت کند.' : 'Registering the rental contract with ANAF within 30 days is the landlord\'s legal obligation, not the tenant\'s; failing to do so creates tax risk for the landlord themselves. We didn\'t find a clear official source on whether an unregistered contract can still be used for your address registration with the immigration authority — to be safe, it\'s best to insist your landlord registers the contract with ANAF.'
            },
            {
              q: currentLang === 'fa' ? 'می‌توانم مالیات بر ارزش افزوده (VAT/TVA) خریدهایم را هنگام خروج از رومانی پس بگیرم؟' : 'Can I get a VAT refund on purchases when leaving Romania?',
              a: currentLang === 'fa' ? 'بله، از طریق اپراتورهای مجاز «Tax Free» (مانند Ask for Tax Free) برای خریدهای بالای ۱۷۵ یورو (شامل مالیات) در هر فاکتور، به شرطی که ظرف ۹۰ روز کالا را از کشور خارج کنید و فاکتور را در گمرک مهر بزنید. کارمزد این اپراتورها حدود ۶٪ به‌علاوه‌ی هزینه‌ی بانکی از مبلغ بازپرداختی کسر می‌شود.' : 'Yes, through authorized "Tax Free" operators (e.g., Ask for Tax Free) for purchases over €175 (VAT included) per invoice, provided you export the goods within 90 days and get the invoice stamped at customs. These operators deduct roughly a 6% commission plus banking fees from the refund.'
            },
            {
              q: currentLang === 'fa' ? 'کد CNP چیست و چطور آن را دریافت کنم؟' : 'What is a CNP and how do I get one?',
              a: currentLang === 'fa' ? 'کد CNP (کد عددی شخصی) شناسه یکتای هر فرد در رومانی است که به‌محض دریافت کارت اقامت، به‌طور خودکار به شما اختصاص داده می‌شود و روی خودِ کارت چاپ می‌شود — نیازی به درخواست جداگانه نیست. این کد برای امور مالیاتی، بانکی، درمانی و بسیاری از خدمات رسمی دیگر الزامی است.' : 'The CNP is your unique personal identification number in Romania, automatically assigned when you receive your residence card and printed on the card itself — no separate application is needed. It\'s required for tax, banking, healthcare, and many other official services.'
            }
          ]} />
