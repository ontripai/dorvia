      ...
      return (
        <div className="space-y-10 animate-fadeIn max-w-[1280px] mx-auto px-4 py-8">
          ...
          <div className="mt-12 bg-[#F8FAFC] rounded-2xl p-6 sm:p-8 border border-[#e2e8f0]">
            ...
            <div className="space-y-6">
              {/* Source: OMAI 163/2011 — exam exemption based on issuing country */}
              <div>
                <h4 className="font-bold text-[#334155] mb-2">{currentLang === 'fa' ? 'آیا برای تبدیل گواهینامه ایرانی به رومانیایی، آزمون رانندگی لازم است؟' : 'Is a driving exam required to convert an Iranian license to a Romanian one?'}</h4>
                <p className="text-sm text-[#475569]">{currentLang === 'fa' ? 'خیر. تبدیل گواهینامه ایرانی به رومانیایی به‌صورت اداری و بدون نیاز به آزمون تئوری یا عملی انجام می‌شود، چون ایران جزو کشورهایی است که طبق OMAI 163/2011 از این معافیت برخوردارند.' : 'No. Converting an Iranian license to a Romanian one is done administratively, without a theory or practical exam, because Iran is among the countries exempted under OMAI 163/2011.'}</p>
              </div>
              {/* Source: hub.mai.gov.ro/serviciu/view?id=73 */}
              <div>
                <h4 className="font-bold text-[#334155] mb-2">{currentLang === 'fa' ? 'کل فرآیند تبدیل واقعاً چقدر طول می‌کشد؟' : 'How long does the entire conversion process actually take?'}</h4>
                <p className="text-sm text-[#475569]">{currentLang === 'fa' ? 'طبق سامانه خدمات رسمی وزارت کشور رومانی (MAI)، زمان صدور گواهینامه‌ی جدید پس از تکمیل و تحویل کامل پرونده، به‌طور میانگین ۵ روز کاری و حداکثر ۱۴ روز تقویمی است. اما این فقط زمان «پردازش اداری» است؛ زمان کلی که کاربران واقعی تجربه می‌کنند (شامل گرفتن نوبت، ترجمه و تایید مدارک، و تایید اصالت گواهینامه توسط مرجع رومانیایی) می‌تواند به یک تا چند ماه برسد. بهتر است این دو زمان را جدا از هم در نظر بگیرید.' : 'According to Romania\'s official government service portal (MAI), once your file is complete and submitted, the license is typically issued within an average of 5 working days, maximum 14 calendar days. However, that is only the "administrative processing" time. The total real-world time — including getting an appointment, translating/certifying documents, and verification of your original license\'s authenticity — can take anywhere from one to several months. It\'s best to think of these as two separate clocks.'}</p>
              </div>
              <div>
                <h4 className="font-bold text-[#334155] mb-2">{currentLang === 'fa' ? 'آیا برای تبدیل گواهینامه به گواهینامه بین‌المللی (IDP) نیاز دارم؟' : 'Do I need an International Driving Permit (IDP) to convert my license?'}</h4>
                <p className="text-sm text-[#475569]">{currentLang === 'fa' ? 'خیر، گواهینامه بین‌المللی (IDP) جزو مدارک لازم برای فرآیند تبدیل گواهینامه ایرانی به رومانیایی نیست. IDP یک مدرک جداگانه است که فقط اگر بخواهید پیش از تبدیل، موقتاً در رومانی یا کشورهای دیگر با گواهینامه ایرانی‌تان رانندگی کنید کاربرد دارد. در صورت شک، بهتر است با اداره‌ی محلی (سرویس مربوطه در استان خود) هم تماس بگیرید.' : 'No, an International Driving Permit (IDP) is not part of the required documents for converting your Iranian license to a Romanian one. An IDP is a separate document, only relevant if you want to temporarily drive in Romania or elsewhere using your original Iranian license before conversion. If in doubt, it\'s a good idea to also confirm with your local county service.'}</p>
              </div>
              <div>
                <h4 className="font-bold text-[#334155] mb-2">{currentLang === 'fa' ? 'گواهینامه‌های جدید ایرانی که پشت آنها به انگلیسی هم نوشته شده، باز هم نیاز به ترجمه رسمی دارند؟' : 'Do newer Iranian licenses with English text on the back still require a certified translation?'}</h4>
                <p className="text-sm text-[#475569]">{currentLang === 'fa' ? 'طبق فهرست رسمی مدارک لازم، ترجمه‌ی رسمی و تاییدشده‌ی گواهینامه همچنان یکی از مدارک الزامی اعلام‌شده است، صرف‌نظر از اینکه پشت گواهینامه متن انگلیسی داشته باشد یا نه. توصیه می‌شود پیش از مراجعه، حتماً با سرویس مربوطه در استان خود تماس بگیرید و مطمئن شوید که آیا در عمل نسخه‌ی انگلیسی پذیرفته می‌شود یا خیر — این موضوع می‌تواند بین ادارات مختلف کمی متفاوت باشد.' : 'Based on the official list of required documents, a certified Romanian translation of the license remains a listed requirement, regardless of whether the back of the license includes English text. We recommend contacting your local county service in advance to confirm whether the English-language version would be accepted in practice — this can vary somewhat between offices.'}</p>
              </div>
              <div>
                <h4 className="font-bold text-[#334155] mb-2">{currentLang === 'fa' ? 'گواهینامه ایرانی‌ام که هنگام تبدیل تحویل داده می‌شود، بعداً قابل بازپس‌گیری است؟' : 'Is my original Iranian license retrievable after being surrendered during conversion?'}</h4>
                <p className="text-sm text-[#475569]">{currentLang === 'fa' ? 'طبق مدارک رسمی رومانیایی، گواهینامه ایرانی شما هنگام تبدیل نزد مرجع رومانیایی نگهداری می‌شود و نمی‌توانید همزمان هر دو گواهینامه را داشته باشید. اما روال بازپس‌گیری یا تحویل آن به سفارت/پلیس ایران، بخشی از فرآیند رسمی رومانیایی نیست و در منابع رسمی رومانی مستند نشده — این موضوع در حوزه‌ی کاری سفارت/کنسولگری ایران است، پس بهتر است مستقیماً با سفارت ایران در بخارست تماس بگیرید.' : 'According to official Romanian documentation, your Iranian license is retained by the Romanian authority during conversion, and you cannot hold both licenses at once. However, the process for retrieving it or having it returned to the Iranian embassy/police is not part of Romania\'s official process and isn\'t documented in Romanian official sources — this falls under the Iranian embassy/consulate\'s own procedures, so it\'s best to contact the Iranian Embassy in Bucharest directly.'}</p>
              </div>
              {/* Source: Directive 2006/126/EC, Art. 2(1) — eur-lex.europa.eu/eli/dir/2006/126/2020-11-01/eng */}
              <div>
                <h4 className="font-bold text-[#334155] mb-2">{currentLang === 'fa' ? 'آیا گواهینامه رومانیایی که از تبدیل گواهینامه ایرانی گرفته‌ام، در سایر کشورهای اتحادیه اروپا (مثل ایتالیا) هم معتبر است؟' : 'Is a Romanian license obtained through converting an Iranian license valid in other EU countries (such as Italy)?'}</h4>
                <p className="text-sm text-[#475569]">{currentLang === 'fa' ? 'بله. طبق ماده ۲ بند ۱ دستورالعمل اتحادیه اروپا ۲۰۰۶/۱۲۶/EC، گواهینامه‌های رانندگی صادرشده توسط هر کشور عضو اتحادیه اروپا، از جمله رومانی، به‌طور متقابل در تمام کشورهای عضو دیگر به رسمیت شناخته می‌شوند. یعنی وقتی گواهینامه‌ی ایرانی شما به گواهینامه‌ی رومانیایی تبدیل شد، دیگر نیازی به تبدیل مجدد در سایر کشورهای اتحادیه اروپا نیست.' : 'Yes. Under Article 2(1) of EU Directive 2006/126/EC, driving licenses issued by any EU member state, including Romania, are mutually recognized across all other member states. This means that once your Iranian license has been converted into a Romanian one, you do not need to convert it again in other EU countries.'}</p>
              </div>
              <div>
                <h4 className="font-bold text-[#334155] mb-2">{currentLang === 'fa' ? 'تبدیل گواهینامه موتورسیکلت یا گواهینامه‌های ویژه (مثل لیفتراک) هم به همین شکل است؟' : 'Does the same conversion process apply to motorcycle or special licenses (such as forklift operation)?'}</h4>
                <p className="text-sm text-[#475569]">{currentLang === 'fa' ? 'این راهنما عمدتاً فرآیند تبدیل گواهینامه‌ی استاندارد خودرو (رده B) را پوشش می‌دهد. سایر رده‌ها (مثل موتورسیکلت یا گواهینامه‌های صنعتی/ویژه مانند لیفتراک) معمولاً از همان اصل معافیت مبتنی بر کشور صادرکننده (OMAI 163/2011) پیروی می‌کنند، اما بهتر است شرایط دقیق رده‌ی خودتان را مستقیماً با سرویس مربوطه در استان محل زندگی‌تان تایید کنید، چون جزئیات ممکن است بین رده‌ها متفاوت باشد.' : 'This guide primarily covers the standard car license (Category B) conversion process. Other categories (such as motorcycle licenses or special/industrial licenses like forklift operation) generally follow the same country-of-issue-based exemption principle under OMAI 163/2011, but we recommend confirming the exact requirements for your specific category directly with your local county service, as details can vary between categories.'}</p>
              </div>
            </div>
          </div>
          ...
          <FaqSchema items={[
            {
              q: currentLang === 'fa' ? 'آیا برای تبدیل گواهینامه ایرانی به رومانیایی، آزمون رانندگی لازم است؟' : 'Is a driving exam required to convert an Iranian license to a Romanian one?',
              a: currentLang === 'fa' ? 'خیر. تبدیل گواهینامه ایرانی به رومانیایی به‌صورت اداری و بدون نیاز به آزمون تئوری یا عملی انجام می‌شود، چون ایران جزو کشورهایی است که طبق OMAI 163/2011 از این معافیت برخوردارند.' : 'No. Converting an Iranian license to a Romanian one is done administratively, without a theory or practical exam, because Iran is among the countries exempted under OMAI 163/2011.'
            },
            {
              q: currentLang === 'fa' ? 'کل فرآیند تبدیل واقعاً چقدر طول می‌کشد؟' : 'How long does the entire conversion process actually take?',
              a: currentLang === 'fa' ? 'طبق سامانه خدمات رسمی وزارت کشور رومانی (MAI)، زمان صدور گواهینامه‌ی جدید پس از تکمیل و تحویل کامل پرونده، به‌طور میانگین ۵ روز کاری و حداکثر ۱۴ روز تقویمی است. اما این فقط زمان «پردازش اداری» است؛ زمان کلی که کاربران واقعی تجربه می‌کنند (شامل گرفتن نوبت، ترجمه و تایید مدارک، و تایید اصالت گواهینامه توسط مرجع رومانیایی) می‌تواند به یک تا چند ماه برسد. بهتر است این دو زمان را جدا از هم در نظر بگیرید.' : 'According to Romania\'s official government service portal (MAI), once your file is complete and submitted, the license is typically issued within an average of 5 working days, maximum 14 calendar days. However, that is only the "administrative processing" time. The total real-world time — including getting an appointment, translating/certifying documents, and verification of your original license\'s authenticity — can take anywhere from one to several months. It\'s best to think of these as two separate clocks.'
            },
            {
              q: currentLang === 'fa' ? 'آیا برای تبدیل گواهینامه به گواهینامه بین‌المللی (IDP) نیاز دارم؟' : 'Do I need an International Driving Permit (IDP) to convert my license?',
              a: currentLang === 'fa' ? 'خیر، گواهینامه بین‌المللی (IDP) جزو مدارک لازم برای فرآیند تبدیل گواهینامه ایرانی به رومانیایی نیست. IDP یک مدرک جداگانه است که فقط اگر بخواهید پیش از تبدیل، موقتاً در رومانی یا کشورهای دیگر با گواهینامه ایرانی‌تان رانندگی کنید کاربرد دارد. در صورت شک، بهتر است با اداره‌ی محلی (سرویس مربوطه در استان خود) هم تماس بگیرید.' : 'No, an International Driving Permit (IDP) is not part of the required documents for converting your Iranian license to a Romanian one. An IDP is a separate document, only relevant if you want to temporarily drive in Romania or elsewhere using your original Iranian license before conversion. If in doubt, it\'s a good idea to also confirm with your local county service.'
            },
            {
              q: currentLang === 'fa' ? 'گواهینامه‌های جدید ایرانی که پشت آنها به انگلیسی هم نوشته شده، باز هم نیاز به ترجمه رسمی دارند؟' : 'Do newer Iranian licenses with English text on the back still require a certified translation?',
              a: currentLang === 'fa' ? 'طبق فهرست رسمی مدارک لازم، ترجمه‌ی رسمی و تاییدشده‌ی گواهینامه همچنان یکی از مدارک الزامی اعلام‌شده است، صرف‌نظر از اینکه پشت گواهینامه متن انگلیسی داشته باشد یا نه. توصیه می‌شود پیش از مراجعه، حتماً با سرویس مربوطه در استان خود تماس بگیرید و مطمئن شوید که آیا در عمل نسخه‌ی انگلیسی پذیرفته می‌شود یا خیر — این موضوع می‌تواند بین ادارات مختلف کمی متفاوت باشد.' : 'Based on the official list of required documents, a certified Romanian translation of the license remains a listed requirement, regardless of whether the back of the license includes English text. We recommend contacting your local county service in advance to confirm whether the English-language version would be accepted in practice — this can vary somewhat between offices.'
            },
            {
              q: currentLang === 'fa' ? 'گواهینامه ایرانی‌ام که هنگام تبدیل تحویل داده می‌شود، بعداً قابل بازپس‌گیری است؟' : 'Is my original Iranian license retrievable after being surrendered during conversion?',
              a: currentLang === 'fa' ? 'طبق مدارک رسمی رومانیایی، گواهینامه ایرانی شما هنگام تبدیل نزد مرجع رومانیایی نگهداری می‌شود و نمی‌توانید همزمان هر دو گواهینامه را داشته باشید. اما روال بازپس‌گیری یا تحویل آن به سفارت/پلیس ایران، بخشی از فرآیند رسمی رومانیایی نیست و در منابع رسمی رومانی مستند نشده — این موضوع در حوزه‌ی کاری سفارت/کنسولگری ایران است، پس بهتر است مستقیماً با سفارت ایران در بخارست تماس بگیرید.' : 'According to official Romanian documentation, your Iranian license is retained by the Romanian authority during conversion, and you cannot hold both licenses at once. However, the process for retrieving it or having it returned to the Iranian embassy/police is not part of Romania\'s official process and isn\'t documented in Romanian official sources — this falls under the Iranian embassy/consulate\'s own procedures, so it\'s best to contact the Iranian Embassy in Bucharest directly.'
            },
            {
              q: currentLang === 'fa' ? 'آیا گواهینامه رومانیایی که از تبدیل گواهینامه ایرانی گرفته‌ام، در سایر کشورهای اتحادیه اروپا (مثل ایتالیا) هم معتبر است؟' : 'Is a Romanian license obtained through converting an Iranian license valid in other EU countries (such as Italy)?',
              a: currentLang === 'fa' ? 'بله. طبق ماده ۲ بند ۱ دستورالعمل اتحادیه اروپا ۲۰۰۶/۱۲۶/EC، گواهینامه‌های رانندگی صادرشده توسط هر کشور عضو اتحادیه اروپا، از جمله رومانی، به‌طور متقابل در تمام کشورهای عضو دیگر به رسمیت شناخته می‌شوند. یعنی وقتی گواهینامه‌ی ایرانی شما به گواهینامه‌ی رومانیایی تبدیل شد، دیگر نیازی به تبدیل مجدد در سایر کشورهای اتحادیه اروپا نیست.' : 'Yes. Under Article 2(1) of EU Directive 2006/126/EC, driving licenses issued by any EU member state, including Romania, are mutually recognized across all other member states. This means that once your Iranian license has been converted into a Romanian one, you do not need to convert it again in other EU countries.'
            },
            {
              q: currentLang === 'fa' ? 'تبدیل گواهینامه موتورسیکلت یا گواهینامه‌های ویژه (مثل لیفتراک) هم به همین شکل است؟' : 'Does the same conversion process apply to motorcycle or special licenses (such as forklift operation)?',
              a: currentLang === 'fa' ? 'این راهنما عمدتاً فرآیند تبدیل گواهینامه‌ی استاندارد خودرو (رده B) را پوشش می‌دهد. سایر رده‌ها (مثل موتورسیکلت یا گواهینامه‌های صنعتی/ویژه مانند لیفتراک) معمولاً از همان اصل معافیت مبتنی بر کشور صادرکننده (OMAI 163/2011) پیروی می‌کنند، اما بهتر است شرایط دقیق رده‌ی خودتان را مستقیماً با سرویس مربوطه در استان محل زندگی‌تان تایید کنید، چون جزئیات ممکن است بین رده‌ها متفاوت باشد.' : 'This guide primarily covers the standard car license (Category B) conversion process. Other categories (such as motorcycle licenses or special/industrial licenses like forklift operation) generally follow the same country-of-issue-based exemption principle under OMAI 163/2011, but we recommend confirming the exact requirements for your specific category directly with your local county service, as details can vary between categories.'
            }
          ]} />
