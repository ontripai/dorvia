...
export const OfficialResourceCard: React.FC<OfficialResourceCardProps> = ({
  ...
  return (
    <a
      ...
      className="editorial-card p-5 bg-white border border-[#dfe6ef] flex flex-col justify-between space-y-4 hover:border-[#0038a8] group transition-all"
    >
      <div className="space-y-2">
        <div className="flex items-center justify-between text-xs text-[#788697]">
          ...
          <ExternalLink size={14} className="group-hover:text-[#0038a8] transition-colors" />
        </div>

        <div className="flex items-start space-x-3 rtl:space-x-reverse pt-1">
          <div className="w-9 h-9 rounded-xl bg-blue-50 text-[#0038a8] flex items-center justify-center shrink-0">
            <Landmark size={18} />
          </div>
          <div>
            <h4 className="font-extrabold text-sm text-[#142033] group-hover:text-[#0038a8] transition-colors leading-snug">
              {title}
            </h4>
            <span className="text-[11px] text-[#0038a8] font-semibold">{domain}</span>
          </div>
        </div>
      </div>
