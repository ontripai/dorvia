...
import { ChevronDown, Menu, Search, LogIn } from './Icons';
    ...
    <header className="fixed top-0 inset-x-0 z-50 bg-white/95 backdrop-blur-md border-b border-[#dfe6ef] h-[80px] flex flex-col justify-between transition-all duration-300">
      ...
      <div className="max-w-[1280px] w-full mx-auto px-4 sm:px-6 lg:px-8 h-full flex items-center justify-between">
        ...
        <div className="hidden lg:flex items-center space-x-3 rtl:space-x-reverse">
          ...
          {/* Customer Portal Login Button (Desktop) */}
          {!isAuthOrPortalPage && (
            <Link
              href={`/${currentLang}/portal/login`}
              className="flex items-center space-x-1.5 rtl:space-x-reverse px-3.5 py-1.5 h-[44px] rounded-xl border border-[#dfe6ef] bg-[#eef3f8] text-[#142033] hover:bg-[#dfe6ef] hover:text-[#2F6FED] transition-all cursor-pointer text-xs font-bold shrink-0"
              aria-label={currentLang === 'fa' ? 'ورود به پورتال متقاضیان' : 'Client Login'}
              title={currentLang === 'fa' ? 'ورود' : 'Login'}
            >
              <LogIn size={16} className="text-[#2F6FED]" />
        ...
        <div className="flex items-center space-x-1 sm:space-x-2 rtl:space-x-reverse lg:hidden shrink-0">
          ...
          {!isAuthOrPortalPage && (
            <Link
              href={`/${currentLang}/portal/login`}
              className="p-2 rounded-lg border border-[#dfe6ef] bg-[#eef3f8] text-[#142033] hover:bg-[#dfe6ef] min-h-[44px] min-w-[44px] flex items-center justify-center transition-colors"
              aria-label={currentLang === 'fa' ? 'ورود به پورتال متقاضیان' : 'Client Login'}
              title={currentLang === 'fa' ? 'ورود' : 'Login'}
            >
              <LogIn size={18} className="text-[#2F6FED]" />
            </Link>
          )}
