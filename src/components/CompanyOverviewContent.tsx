        ...
        <div className="space-y-10 animate-fadeIn max-w-[1280px] mx-auto px-4 py-8">
          ...
          <div className="p-4 bg-amber-50 border border-amber-200 rounded-2xl text-xs text-amber-900 leading-relaxed font-semibold">
            ⚠️ {currentLang === 'fa'
              ...
              : "Important update (Law 239/2025, effective December 18, 2025): the minimum SRL share capital, which had been a purely symbolic 1 RON since 2020, was raised to 500 RON (5,000 RON for companies with annual turnover above 400,000 RON). Companies registered before this date have until December 18, 2027 to bring their capital up to the new threshold. Also effective January 1, 2026, every newly registered SRL must open a payment account (cont de plăți, at a bank or the State Treasury) within 60 working days of registration and report it to the tax authority (ANAF) via the SPV portal within 15 days of opening it — failure to comply can trigger a fine of 3,000 to 10,000 RON and the company being declared \"fiscally inactive\". A note on fintech accounts: because the law's text uses the broader term \"payment account\" rather than just \"credit institution,\" Revolut Business (which sits under a genuinely licensed European bank, Revolut Bank UAB) very likely satisfies this requirement; the real remaining ambiguity is narrower, limited to electronic-money institutions without a banking license (e.g. Wise Business), which no official ANAF notice has yet clarified — before relying on such an account for this specific obligation, confirm directly with your accountant or ANAF."}
          </div>
          ...
          <div className="mt-12 bg-[#F8FAFC] rounded-2xl p-6 sm:p-8 border border-[#e2e8f0]">
            ...
            <div className="space-y-6">
              ...
              <div id="srl-vs-pfa">
                <h4 className="font-bold text-[#334155] mb-2">{currentLang === 'fa' ? 'تفاوت شرکت مسئولیت محدود (SRL) با فریلنسری یا خوداشتغالی (PFA) چیست و کدام برای من مناسب‌تر است؟' : 'What is the difference between a limited liability company (SRL) and sole proprietorship (PFA), and which is right for me?'}</h4>
                <p className="text-sm text-[#475569]">{currentLang === 'fa' ? 'باید به سه تمایز کلیدی توجه کنید: ۱. محدودیت ملیت: بر اساس بررسی ما، طبق ماده ۳(۱) فرمان اضطراری OUG 44/2008، ثبت PFA به اتباع رومانی، کشورهای عضو اتحادیه اروپا (EU) و منطقه اقتصادی اروپا (EEA) محدود است. بنابراین PFA عموماً برای شهروندان ایرانی در دسترس نیست و مسیر عملی، ثبت SRL است. اینکه داشتن اقامت بلندمدت رومانی این مانع را برمی‌دارد یا نه، در منابعی که بررسی کردیم روشن نشد و نظر کارشناسان متفاوت بود — اگر مشخصاً دنبال PFA هستید، پیش از هر اقدامی این را با یک وکیل یا مستقیماً از ONRC بپرسید و روی فرض «حتماً می‌شود» برنامه‌ریزی نکنید. ۲. مسئولیت مالی: در SRL مسئولیت اصولاً محدود به آورده‌ی شماست، اما این سپر مطلق نیست: مدیر می‌تواند در فرآیند ورشکستگی شخصاً مسئول شناخته شود، بدهی مالیاتی در شرایطی به مدیر یا سهامدار تسری پیدا می‌کند، و بانک‌ها و موجران معمولاً ضمانت شخصی می‌خواهند که عملاً همان محدودیت را دور می‌زند؛ در PFA مسئولیت نامحدود است و تمام دارایی شخصی و خانوادگی پاسخگوی بدهی‌هاست. ۳. بار اداری: PFA ثبت و حسابداری ساده‌تری دارد اما محدودیت مقیاس دارد؛ SRL نیاز به حسابداری دوبل رسمی تحت نظارت حسابدار عضو CECCAR دارد اما امکان استخدام نامحدود، اعتبار تجاری بالاتر و انعطاف‌پذیری کامل برای فعالیت بین‌المللی فراهم می‌کند.' : 'You should consider three key distinctions: 1. Nationality restriction: Based on our review, Article 3(1) of Emergency Ordinance OUG 44/2008 restricts PFA registration to citizens of Romania, EU member states, and the European Economic Area (EEA). Consequently, a PFA is generally not accessible to Iranian nationals and the practical route is an SRL. Whether holding Romanian long-term residence removes this barrier was not settled in the sources we reviewed, and practitioner opinion differed — if you specifically want a PFA, confirm with a lawyer or directly with ONRC before committing, and don\'t plan on the assumption that it will work. 2. Financial liability: In an SRL, liability is in principle limited to your contribution — but that shield is not absolute: a director can be held personally liable in insolvency proceedings, tax debts can in certain circumstances be extended to directors or shareholders, and banks and landlords routinely require a personal guarantee, which contracts around the limit in practice; in a PFA, liability is unlimited, placing personal and family assets at risk for business debts. 3. Administrative burden: A PFA has simpler bookkeeping but scalability limits; an SRL requires formal double-entry accounting managed by a CECCAR accountant, but provides unlimited hiring ability, higher commercial credibility, and full flexibility for international operations.'}</p>
              </div>
              <div>
                <h4 className="font-bold text-[#334155] mb-2">{currentLang === 'fa' ? 'مالیات شرکت (SRL) در سال ۲۰۲۶ چقدر است و رژیم مالیاتی میکرو چه شرایطی دارد؟' : 'What are corporate taxes for an SRL in 2026, and what are the micro-enterprise conditions?'}</h4>
                <p className="text-sm text-[#475569]">{currentLang === 'fa' ? 'رژیم‌های اصلی مالیاتی شرکت‌ها در سال ۲۰۲۶ به این ترتیب است: ۱. مالیات میکرو (شرکت‌های کوچک): نرخ واحد ۱٪ بر درآمد کل اعمال می‌شود. شرایط بهره‌مندی از این رژیم شامل سقف گردش مالی سالانه ۱۰۰,۰۰۰ یورو و داشتن حداقل یک کارمند تمام‌وقت با قرارداد معتبر است. ۲. مالیات بر سود استاندارد: در صورت عبور از سقف گردش مالی یا نداشتن کارمند، مالیات با نرخ استاندارد ۱۶٪ بر سود خالص محاسبه می‌شود. ۳. سرمایه ثبتی: حداقل سرمایه ثبتی ۵۰۰ لئو (۵,۰۰۰ لئو در صورت گردش مالی بیش از ۴۰۰,۰۰۰ لئو) است. (توجه: توزیع سود سهام مشمول مقررات مالیاتی جداگانه است و برای تعیین دقیق وضعیت پرونده خود حتماً با حسابدار رسمی مشورت کنید).' : 'The main corporate tax regimes in 2026 are: 1. Micro-enterprise regime: A unified 1% tax on total revenue applies. Eligibility requires an annual turnover ceiling of €100,000 and at least one full-time employee under a valid contract. 2. Standard corporate profit tax: If turnover exceeds the ceiling or the company has no employees, tax is calculated at the standard 16% on net profit. 3. Share capital: Minimum share capital is 500 RON (5,000 RON if turnover exceeds 400,000 RON). (Note: Dividend distributions are subject to separate fiscal rules; consult a certified accountant for your specific situation).'}</p>
              </div>
              <div>
                <h4 className="font-bold text-[#334155] mb-2">{currentLang === 'fa' ? 'آیا استخدام حسابدار برای شرکت الزامی است و چگونه اعتبار یک حسابدار رسمی (CECCAR) را استعلام کنم؟' : 'Is hiring an accountant mandatory for an SRL, and how can I verify a CECCAR-certified accountant?'}</h4>
                <p className="text-sm text-[#475569]">{currentLang === 'fa' ? 'بله، بر اساس قانون حسابداری رومانی (Legea 82/1991)، اسناد مالی و اظهارنامه‌های یک شرکت باید توسط کارشناس حسابداری مجاز یا حسابدار خبره تایید و تنظیم شوند. برای راستی‌آزمایی: ۱. تابلوی اعضا: کارشناسان رسمی در سامانه تابلوی اعضای کانون کارشناسان حسابداری و حسابداران رسمی رومانی (Tabloul membrilor CECCAR) در وب‌سایت ceccar.ro ثبت هستند. ۲. مدارک حرفه‌ای: شماره عضویت، وضعیت فعال بودن پروانه در سال جاری و کارنه عضویت (Carnet de membru) را بررسی کنید. ۳. هزینه‌ها: تعرفه قانونی ثابتی از سوی دولت تعیین نشده است و دستمزد بر اساس حجم اسناد، پرسنل و گردش مالی توافق می‌شود؛ بنابراین توصیه می‌شود پیش از عقد قرارداد از حداقل سه حسابدار یا موسسه معتبر پیش‌فاکتور دریافت کنید.' : 'Yes, under Romanian Accounting Law (Legea 82/1991), corporate financial statements and tax filings must be prepared or supervised by an authorized accounting expert or certified accountant. To verify: 1. Member Registry: Certified professionals are listed in the official register (Tabloul membrilor CECCAR) on ceccar.ro. 2. Professional credentials: Check their membership number, active license status for the current year, and membership card (Carnet de membru). 3. Service fees: There is no government-mandated fixed tariff; fees depend on transaction volume, staff count, and turnover, so getting written quotes from at least three licensed accountants is recommended before signing.'}</p>
              </div>
            </div>
          </div>
          ...
          <FaqSchema items={[
            ...
            {
              q: currentLang === 'fa' ? 'تفاوت شرکت مسئولیت محدود (SRL) با فریلنسری یا خوداشتغالی (PFA) چیست و کدام برای من مناسب‌تر است؟' : 'What is the difference between a limited liability company (SRL) and sole proprietorship (PFA), and which is right for me?',
              a: currentLang === 'fa' ? 'باید به سه تمایز کلیدی توجه کنید: ۱. محدودیت ملیت: بر اساس بررسی ما، طبق ماده ۳(۱) فرمان اضطراری OUG 44/2008، ثبت PFA به اتباع رومانی، کشورهای عضو اتحادیه اروپا (EU) و منطقه اقتصادی اروپا (EEA) محدود است. بنابراین PFA عموماً برای شهروندان ایرانی در دسترس نیست و مسیر عملی، ثبت SRL است. اینکه داشتن اقامت بلندمدت رومانی این مانع را برمی‌دارد یا نه، در منابعی که بررسی کردیم روشن نشد و نظر کارشناسان متفاوت بود — اگر مشخصاً دنبال PFA هستید، پیش از هر اقدامی این را با یک وکیل یا مستقیماً از ONRC بپرسید و روی فرض «حتماً می‌شود» برنامه‌ریزی نکنید. ۲. مسئولیت مالی: در SRL مسئولیت اصولاً محدود به آورده‌ی شماست، اما این سپر مطلق نیست: مدیر می‌تواند در فرآیند ورشکستگی شخصاً مسئول شناخته شود، بدهی مالیاتی در شرایطی به مدیر یا سهامدار تسری پیدا می‌کند، و بانک‌ها و موجران معمولاً ضمانت شخصی می‌خواهند که عملاً همان محدودیت را دور می‌زند؛ در PFA مسئولیت نامحدود است و تمام دارایی شخصی و خانوادگی پاسخگوی بدهی‌هاست. ۳. بار اداری: PFA ثبت و حسابداری ساده‌تری دارد اما محدودیت مقیاس دارد؛ SRL نیاز به حسابداری دوبل رسمی تحت نظارت حسابدار عضو CECCAR دارد اما امکان استخدام نامحدود، اعتبار تجاری بالاتر و انعطاف‌پذیری کامل برای فعالیت بین‌المللی فراهم می‌کند.' : 'You should consider three key distinctions: 1. Nationality restriction: Based on our review, Article 3(1) of Emergency Ordinance OUG 44/2008 restricts PFA registration to citizens of Romania, EU member states, and the European Economic Area (EEA). Consequently, a PFA is generally not accessible to Iranian nationals and the practical route is an SRL. Whether holding Romanian long-term residence removes this barrier was not settled in the sources we reviewed, and practitioner opinion differed — if you specifically want a PFA, confirm with a lawyer or directly with ONRC before committing, and don\'t plan on the assumption that it will work. 2. Financial liability: In an SRL, liability is in principle limited to your contribution — but that shield is not absolute: a director can be held personally liable in insolvency proceedings, tax debts can in certain circumstances be extended to directors or shareholders, and banks and landlords routinely require a personal guarantee, which contracts around the limit in practice; in a PFA, liability is unlimited, placing personal and family assets at risk for business debts. 3. Administrative burden: A PFA has simpler bookkeeping but scalability limits; an SRL requires formal double-entry accounting managed by a CECCAR accountant, but provides unlimited hiring ability, higher commercial credibility, and full flexibility for international operations.'
            },
            {
              q: currentLang === 'fa' ? 'مالیات شرکت (SRL) در سال ۲۰۲۶ چقدر است و رژیم مالیاتی میکرو چه شرایطی دارد؟' : 'What are corporate taxes for an SRL in 2026, and what are the micro-enterprise conditions?',
              a: currentLang === 'fa' ? 'رژیم‌های اصلی مالیاتی شرکت‌ها در سال ۲۰۲۶ به این ترتیب است: ۱. مالیات میکرو (شرکت‌های کوچک): نرخ واحد ۱٪ بر درآمد کل اعمال می‌شود. شرایط بهره‌مندی از این رژیم شامل سقف گردش مالی سالانه ۱۰۰,۰۰۰ یورو و داشتن حداقل یک کارمند تمام‌وقت با قرارداد معتبر است. ۲. مالیات بر سود استاندارد: در صورت عبور از سقف گردش مالی یا نداشتن کارمند، مالیات با نرخ استاندارد ۱۶٪ بر سود خالص محاسبه می‌شود. ۳. سرمایه ثبتی: حداقل سرمایه ثبتی ۵۰۰ لئو (۵,۰۰۰ لئو در صورت گردش مالی بیش از ۴۰۰,۰۰۰ لئو) است. (توجه: توزیع سود سهام مشمول مقررات مالیاتی جداگانه است و برای تعیین دقیق وضعیت پرونده خود حتماً با حسابدار رسمی مشورت کنید).' : 'The main corporate tax regimes in 2026 are: 1. Micro-enterprise regime: A unified 1% tax on total revenue applies. Eligibility requires an annual turnover ceiling of €100,000 and at least one full-time employee under a valid contract. 2. Standard corporate profit tax: If turnover exceeds the ceiling or the company has no employees, tax is calculated at the standard 16% on net profit. 3. Share capital: Minimum share capital is 500 RON (5,000 RON if turnover exceeds 400,000 RON). (Note: Dividend distributions are subject to separate fiscal rules; consult a certified accountant for your specific situation).'
            },
            {
              q: currentLang === 'fa' ? 'آیا استخدام حسابدار برای شرکت الزامی است و چگونه اعتبار یک حسابدار رسمی (CECCAR) را استعلام کنم؟' : 'Is hiring an accountant mandatory for an SRL, and how can I verify a CECCAR-certified accountant?',
              a: currentLang === 'fa' ? 'بله، بر اساس قانون حسابداری رومانی (Legea 82/1991)، اسناد مالی و اظهارنامه‌های یک شرکت باید توسط کارشناس حسابداری مجاز یا حسابدار خبره تایید و تنظیم شوند. برای راستی‌آزمایی: ۱. تابلوی اعضا: کارشناسان رسمی در سامانه تابلوی اعضای کانون کارشناسان حسابداری و حسابداران رسمی رومانی (Tabloul membrilor CECCAR) در وب‌سایت ceccar.ro ثبت هستند. ۲. مدارک حرفه‌ای: شماره عضویت، وضعیت فعال بودن پروانه در سال جاری و کارنه عضویت (Carnet de membru) را بررسی کنید. ۳. هزینه‌ها: تعرفه قانونی ثابتی از سوی دولت تعیین نشده است و دستمزد بر اساس حجم اسناد، پرسنل و گردش مالی توافق می‌شود؛ بنابراین توصیه می‌شود پیش از عقد قرارداد از حداقل سه حسابدار یا موسسه معتبر پیش‌فاکتور دریافت کنید.' : 'Yes, under Romanian Accounting Law (Legea 82/1991), corporate financial statements and tax filings must be prepared or supervised by an authorized accounting expert or certified accountant. To verify: 1. Member Registry: Certified professionals are listed in the official register (Tabloul membrilor CECCAR) on ceccar.ro. 2. Professional credentials: Check their membership number, active license status for the current year, and membership card (Carnet de membru). 3. Service fees: There is no government-mandated fixed tariff; fees depend on transaction volume, staff count, and turnover, so getting written quotes from at least three licensed accountants is recommended before signing.'
            }
          ]} />
        ...
        <div className="space-y-12 animate-fadeIn max-w-[1280px] mx-auto px-4 py-8">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <Link href="/company/investment" className="editorial-card p-4 space-y-2 bg-white cursor-pointer hover:border-[#2F6FED] transition-colors" >
                <h4 className="font-bold text-[#2F6FED]">{currentLang === 'fa' ? 'فرصت‌های سرمایه‌گذاری' : 'Investment Opportunities'}</h4>
                <p className="text-xs text-[#526174]">{currentLang === 'fa' ? 'مسیرهای اقامت از طریق سرمایه‌گذاری (ARICE و بیشتر)' : 'Residency-through-investment pathways (ARICE and more)'}</p>
              </Link>
            </div>
          </div>
        </div>
