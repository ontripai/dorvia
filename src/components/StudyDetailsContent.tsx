        ...
        <div className="space-y-10 animate-fadeIn max-w-[1280px] mx-auto px-4 py-8">
          ...
          <div className="mt-12 bg-[#F8FAFC] rounded-2xl p-6 sm:p-8 border border-[#e2e8f0]">
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <p className="text-sm text-[#475569]">{currentLang === 'fa' ? `این به رشتهی شما بستگی دارد و از دو مرجع رسمی جداگانه در ایران باید استعلام بگیرید، نه یک فهرست عمومی: — برای رشتههای غیرپزشکی: مرجع وزارت علوم، تحقیقات و فناوری است و فهرست دانشگاههای مورد تایید را «سازمان امور دانشجویان» در grad.saorg.ir منتشر میکند. این فهرست سه سطح دارد — گروه الف (ممتاز)، گروه ب (خوب)، گروه ج (متوسط) — و سطح دانشگاه پیامد مستقیم دارد: برای مقطع دکتری باید صرفاً در دانشگاههای گروه الف یا ب ثبتنام کنید، و مدارک دانشگاههای گروه ج فقط تا مقطع کارشناسی ارشد قابل ارزشیابی است. — برای رشتههای پزشکی، دندانپزشکی و داروسازی: مرجع جداگانه، وزارت بهداشت، درمان و آموزش پزشکی است (سامانهی موحد)؛ دانشگاههای پزشکی اساساً در فهرست سازمان امور دانشجویان نمیآیند، چون آن فهرست صرفاً رشتههای در حیطهی وزارت علوم را پوشش میدهد — پس نبودنشان در آن فهرست به معنای تایید نشدن نیست. سه قاعدهای که بیشترین هزینه را برای کسانی داشته که از آنها بیخبر بودهاند: (۱) ملاک، اعتبار دانشگاه در زمان شروع تحصیل شماست و فهرست هر سال در اسفند بازنشر میشود؛ (۲) استعلام باید پیش از پذیرش انجام شود، چون طبق تصریح همان اداره کل هرگونه درخواست بررسی اعتبار در حین تحصیل یا پس از اتمام آن به هیچ عنوان امکانپذیر نیست؛ (۳) معتبر بودن شعبهی اصلی یک دانشگاه به منزلهی تایید شعب آن در کشورهای دیگر نیست. نکتهی آخر: طبق رویهی رایج، برای حفظ اعتبار مدرک باید بخش عمدهی دورهی تحصیلی را در همان دانشگاه تاییدشده گذراند، نه اینکه فقط بخشی از دوره آنجا و بقیه جای دیگر باشد.` : `This depends on your field, and you need to check with two separate official Iranian authorities, not a single generic list: — For non-medical fields: the authority is the Ministry of Science, Research and Technology, and the list of approved universities is published by the Organization for Students Affairs (SAORG) at grad.saorg.ir. That list has three tiers — Group A (excellent), Group B (good), Group C (average) — and the tier has direct consequences: for a doctorate you must enrol only at a Group A or Group B university, and degrees from Group C universities can be evaluated only up to master's level. — For Medicine, Dentistry and Pharmacy: a separate authority, the Ministry of Health and Medical Education (the "Movahed" system); medical universities do not appear on the SAORG list at all, because that list covers only fields within the Ministry of Science's remit — so their absence from it does not mean they are unapproved. Three rules that have cost the most for people who did not know them: (1) what counts is the university's standing at the time you start your studies, and the list is republished every year in Esfand; (2) the check must happen before you accept an offer, because the same office states that any request to review a university's standing during your studies or after you graduate is not possible under any circumstances; (3) a main campus being approved does not mean its branches in other countries are approved. One last point: the common rule is that you must complete the majority of your programme at the approved university itself, not split mostly elsewhere.`}</p>
              </div>
              <div>
                <h4 className="font-bold text-[#334155] mb-2">{currentLang === 'fa' ? 'اگر پزشکی، دندانپزشکی یا داروسازی بخوانم، برای ارزشیابی مدرکم در ایران آزمون لازم است؟' : 'If I study medicine, dentistry or pharmacy, will I need to sit an exam in Iran?'}</h4>
                <p className="text-sm text-[#475569]">{currentLang === 'fa' ? `بله، و این نکته‌ای است که خیلی‌ها دیر متوجهش می‌شوند. طبق سوالات متداول رسمی مرکز خدمات آموزشی وزارت بهداشت، ارزشیابی این سه رشته با آزمون در ایران همراه است: پزشکی عمومی «آزمون پیش‌کارورزی»، داروسازی «آزمون ارزشیابی»، و دندانپزشکی عمومی «آزمون ملی» — معمولاً سالی دو بار. همان منبع می‌گوید بعضی متقاضیان معاف‌اند ولی شرایط معافیت را به آیین‌نامه‌ی ارزشیابی ارجاع می‌دهد؛ ما آن آیین‌نامه را مستقیماً ندیده‌ایم و درباره‌ی معافیت‌ها چیزی نمی‌گوییم. در داروسازی حتی با معافیت از آزمون، ۸ واحد کارآموزی داروخانه الزامی است. دو قاعده‌ی دیگر هم پیش از انتخاب دانشگاه مهم‌اند: دست‌کم نیمی از دوره باید در دانشگاه مورد تایید گذرانده شود، و تحصیلات غیرحضوری و مکاتبه‌ای اصلاً ارزشیابی نمی‌شود. مرجع پاسخ برای وضعیت شخصی شما mohed.behdasht.gov.ir/Assessment است.` : `Yes — and it is something many people learn too late. According to the official FAQ of the Ministry of Health's educational services centre, evaluating these three degrees involves an examination in Iran: the pre-internship exam for general medicine, the evaluation exam for pharmacy, and the national exam for general dentistry, usually held twice a year. The same source says some applicants are exempt but refers the conditions to the evaluation regulation; we have not read that regulation directly and say nothing about exemptions. In pharmacy, even with an exemption from the exam, 8 credits of pharmacy internship are required. Two further rules matter before you choose a university: at least half of the programme must be completed at an approved university, and distance or correspondence study is not evaluated at all. For your own situation the authority is mohed.behdasht.gov.ir/Assessment.`}</p>
              </div>
          ...
          <FaqSchema items={[
            ...
            },
            {
              ...
              a: currentLang === 'fa' ? `این به رشتهی شما بستگی دارد و از دو مرجع رسمی جداگانه در ایران باید استعلام بگیرید، نه یک فهرست عمومی: — برای رشتههای غیرپزشکی: مرجع وزارت علوم، تحقیقات و فناوری است و فهرست دانشگاههای مورد تایید را «سازمان امور دانشجویان» در grad.saorg.ir منتشر میکند. این فهرست سه سطح دارد — گروه الف (ممتاز)، گروه ب (خوب)، گروه ج (متوسط) — و سطح دانشگاه پیامد مستقیم دارد: برای مقطع دکتری باید صرفاً در دانشگاههای گروه الف یا ب ثبتنام کنید، و مدارک دانشگاههای گروه ج فقط تا مقطع کارشناسی ارشد قابل ارزشیابی است. — برای رشتههای پزشکی، دندانپزشکی و داروسازی: مرجع جداگانه، وزارت بهداشت، درمان و آموزش پزشکی است (سامانهی موحد)؛ دانشگاههای پزشکی اساساً در فهرست سازمان امور دانشجویان نمیآیند، چون آن فهرست صرفاً رشتههای در حیطهی وزارت علوم را پوشش میدهد — پس نبودنشان در آن فهرست به معنای تایید نشدن نیست. سه قاعدهای که بیشترین هزینه را برای کسانی داشته که از آنها بیخبر بودهاند: (۱) ملاک، اعتبار دانشگاه در زمان شروع تحصیل شماست و فهرست هر سال در اسفند بازنشر میشود؛ (۲) استعلام باید پیش از پذیرش انجام شود، چون طبق تصریح همان اداره کل هرگونه درخواست بررسی اعتبار در حین تحصیل یا پس از اتمام آن به هیچ عنوان امکانپذیر نیست؛ (۳) معتبر بودن شعبهی اصلی یک دانشگاه به منزلهی تایید شعب آن در کشورهای دیگر نیست. نکتهی آخر: طبق رویهی رایج، برای حفظ اعتبار مدرک باید بخش عمدهی دورهی تحصیلی را در همان دانشگاه تاییدشده گذراند، نه اینکه فقط بخشی از دوره آنجا و بقیه جای دیگر باشد.` : `This depends on your field, and you need to check with two separate official Iranian authorities, not a single generic list: — For non-medical fields: the authority is the Ministry of Science, Research and Technology, and the list of approved universities is published by the Organization for Students Affairs (SAORG) at grad.saorg.ir. That list has three tiers — Group A (excellent), Group B (good), Group C (average) — and the tier has direct consequences: for a doctorate you must enrol only at a Group A or Group B university, and degrees from Group C universities can be evaluated only up to master's level. — For Medicine, Dentistry and Pharmacy: a separate authority, the Ministry of Health and Medical Education (the "Movahed" system); medical universities do not appear on the SAORG list at all, because that list covers only fields within the Ministry of Science's remit — so their absence from it does not mean they are unapproved. Three rules that have cost the most for people who did not know them: (1) what counts is the university's standing at the time you start your studies, and the list is republished every year in Esfand; (2) the check must happen before you accept an offer, because the same office states that any request to review a university's standing during your studies or after you graduate is not possible under any circumstances; (3) a main campus being approved does not mean its branches in other countries are approved. One last point: the common rule is that you must complete the majority of your programme at the approved university itself, not split mostly elsewhere.`
            },
            {
              q: currentLang === 'fa' ? 'اگر پزشکی، دندانپزشکی یا داروسازی بخوانم، برای ارزشیابی مدرکم در ایران آزمون لازم است؟' : 'If I study medicine, dentistry or pharmacy, will I need to sit an exam in Iran?',
              a: currentLang === 'fa' ? `بله، و این نکته‌ای است که خیلی‌ها دیر متوجهش می‌شوند. طبق سوالات متداول رسمی مرکز خدمات آموزشی وزارت بهداشت، ارزشیابی این سه رشته با آزمون در ایران همراه است: پزشکی عمومی «آزمون پیش‌کارورزی»، داروسازی «آزمون ارزشیابی»، و دندانپزشکی عمومی «آزمون ملی» — معمولاً سالی دو بار. همان منبع می‌گوید بعضی متقاضیان معاف‌اند ولی شرایط معافیت را به آیین‌نامه‌ی ارزشیابی ارجاع می‌دهد؛ ما آن آیین‌نامه را مستقیماً ندیده‌ایم و درباره‌ی معافیت‌ها چیزی نمی‌گوییم. در داروسازی حتی با معافیت از آزمون، ۸ واحد کارآموزی داروخانه الزامی است. دو قاعده‌ی دیگر هم پیش از انتخاب دانشگاه مهم‌اند: دست‌کم نیمی از دوره باید در دانشگاه مورد تایید گذرانده شود، و تحصیلات غیرحضوری و مکاتبه‌ای اصلاً ارزشیابی نمی‌شود. مرجع پاسخ برای وضعیت شخصی شما mohed.behdasht.gov.ir/Assessment است.` : `Yes — and it is something many people learn too late. According to the official FAQ of the Ministry of Health's educational services centre, evaluating these three degrees involves an examination in Iran: the pre-internship exam for general medicine, the evaluation exam for pharmacy, and the national exam for general dentistry, usually held twice a year. The same source says some applicants are exempt but refers the conditions to the evaluation regulation; we have not read that regulation directly and say nothing about exemptions. In pharmacy, even with an exemption from the exam, 8 credits of pharmacy internship are required. Two further rules matter before you choose a university: at least half of the programme must be completed at an approved university, and distance or correspondence study is not evaluated at all. For your own situation the authority is mohed.behdasht.gov.ir/Assessment.`
            },
            {
        ...
        <div className="space-y-10 animate-fadeIn max-w-[1280px] mx-auto px-4 py-8">
          ...
          <div className="bg-white p-6 sm:p-8 rounded-2xl border border-[#dfe6ef] shadow-sm space-y-4">
            ...
            <ol className="space-y-2 text-sm text-[#526174] list-decimal list-inside">
              ...
              <li>{currentLang === 'fa' ? 'هزینه ویزا (۳۰۰ یورو از ۲۷ آوریل ۲۰۲۶؛ پیشتر ۱۲۰ یورو بود؛ برای دارندگان بورسیه دولتی رومانی معاف است).' : 'Visa fee (€300 as of April 27, 2026; previously €120; waived for holders of the Romanian government scholarship).'}</li>
            </ol>
            <p className="text-[11px] text-slate-400">
              {currentLang === 'fa' ? 'منبع: mae.ro (چک‌لیست عمومی ویزای D/SD تحصیلی)، تایید‌شده توسط uefiscdi.ro (سازمان دولتی تحقیقات/آموزش رومانی)، دفتر بین‌الملل دانشگاه پلی‌تکنیک تیمیشوارا، و دانشگاه رومانیایی-آمریکایی. مبلغ هزینه ویزا طبق صفحه رسمی کارمزد eviza.mae.ro/VisaFees (۲۷ آوریل ۲۰۲۶) به‌روزرسانی شد؛ درخواست‌های ثبت‌شده پیش از این تاریخ با تعرفه ۱۲۰ یورویی قدیم بررسی می‌شوند.' : 'Source: mae.ro (general D/SD student visa checklist), corroborated by UEFISCDI (Romania\'s state research/education funding agency), Politehnica Timișoara\'s international office, and Romanian-American University. The visa fee figure was updated per the official fee page at eviza.mae.ro/VisaFees (April 27, 2026); applications submitted before that date are processed at the old 120 EUR rate.'}
            </p>
          </div>
