    ...
    case 'citizenship':
      return (
        <div className="space-y-10 animate-fadeIn max-w-[1280px] mx-auto px-4 py-8">
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5 sm:p-6 text-xs sm:text-sm text-amber-950 leading-relaxed">
            {currentLang === 'fa'
              ? 'توجه دقیق‌تر: صفحه رسمی ماده ۸ روی cetatenie.just.ro برخلاف صفحه «ترک تابعیت» (که مبلغ دقیق ۶۰۰ لئو را صریحاً اعلام می‌کند)، برای خودِ «اخذ» تابعیت هیچ مبلغ ثابتی منتشر نکرده — این دو هزینه را با هم اشتباه نگیرید. منابع ثانویه (غیررسمی) ارقام متناقضی ذکر می‌کنند (از رایگان بودن ثبت پرونده تا حدود ۲۰۰ لئو هزینه تمبر/ثبت، تا ~۲۲۰ یورو برای اظهارنامه‌های کنسولی جداگانه) که هیچ‌کدام منبع رسمی مشخصی ندارند. هزینه‌های واقعی شما عمدتاً از ترجمه رسمی، آپوستیل/لژالیزاسیون و اظهارنامه‌های نوتاری خواهد بود، نه یک «عوارض پرونده» ثابت. پیش از اقدام، حتماً در همان لحظه ثبت پرونده مبلغ دقیق را از باجه ANC یا با تماس تلفنی تایید کنید.'
              : "A more precise note: unlike the \"renunciation of citizenship\" page (which explicitly states a fixed 600 RON fee), ANC's own official Article 8 page publishes no fixed fee amount for the acquisition process itself — don't confuse the two. Unofficial secondary sources give conflicting figures (from filing being free, to roughly 200 RON in stamp/registration fees, to about €220 for separate consular declarations), none tied to a clear official source. Your real costs will mostly come from certified translations, apostille/legalization, and notarized declarations rather than one fixed \"application fee.\" Before proceeding, confirm the exact amount at the ANC counter when filing, or by phone."}
          </div>
          ...
          <div className="mt-12 bg-[#F8FAFC] rounded-2xl p-6 sm:p-8 border border-[#e2e8f0]">
            ...
            <div className="space-y-6">
              ...
              <div>
                <h4 className="font-bold text-[#334155] mb-2">{currentLang === 'fa' ? 'آیا پاسپورت رومانی الان بدون ویزا به آمریکا سفر می‌کند؟' : 'Does a Romanian passport currently allow visa-free travel to the US?'}</h4>
